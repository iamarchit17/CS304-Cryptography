...
void longDivision(uint16_t divisor, uint16_t dividend, uint16_t arr[2]){
    //0th index stores remainder
    //1st index stores quotient
    uint16_t x = (uint16_t) 1;

    uint16_t degDivisor = (uint16_t)0;
    uint16_t degDividend = (uint16_t)0;

    for(uint16_t i = 0; i < 16; i++){
        if((divisor >> i) & x) degDivisor = i;
        if((dividend >> i) & x) degDividend = i;
    }
    // printf("%d %d", degDivisor, degDividend);

    while(degDividend >= degDivisor){
        uint16_t shift = degDividend - degDivisor;
        uint16_t prod = divisor << shift;
        arr[1] = arr[1] | (x << shift);
...
//                 uint16_t y =  product(M[j][k], s[k][i]);
//                 x = x ^ y;
//             }
//             //printf("i = %x, j = %x, x = %x\n", i, j, x);
//             uint16_t arr[2];
//             longDivision(G, x, arr);
//             temp[j] = arr[0] & z;
//         }
...
void keyScheduling(uchar_t key[16], uchar_t roundKeys[11][4][4]){
    ...
    for(int i = 4; i < 44; i++){
        ...
        if(i % 4 == 0) temp = subWord(rotWord(temp)) ^ (RCON[i/4 - 1]);
...
int main(){
    ...
    uchar_t key[20]; // = {0x95, 0x65, 0xfd, 0xf3, 0xa0, 0x12, 0x39, 0xcd, 0x29, 0xff, 0xae, 0x28, 0xc2, 0xa9, 0xd1, 0xc4};
